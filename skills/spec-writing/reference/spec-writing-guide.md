...
- More than 15 ACs in one story — **counted as assertions, not bullets, and checked against the
  project's own cap.** Two corrections to the naive reading:
  ...
  - **Use the project's cap, not this number.** Read `config.precheck.storySizeGate.maxAcCount` from
    the project's `.nax/config.json` (e.g. `rs-stock` sets 24) and check against that; 15 is the
    fallback when the project sets none. Corpus check: 6.7% of stories exceed 15 and 0.5% exceed 24,
    so the ceiling binds rarely — but when it does, the run fails a precheck rather than degrading
    quietly.
