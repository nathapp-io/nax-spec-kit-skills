...
6. Verify every seam AC whose stubbed symbol **and** entry point both already exist names an entry point that genuinely reaches it — path present, not gated behind an unestablished flag, and the named method actually called. (If either endpoint is new, the path is being created and there is nothing to check.)
