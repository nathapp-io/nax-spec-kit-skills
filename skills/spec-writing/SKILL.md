...
- **State the composition when a function declares 2+ thresholds, limits or modes** (guide Rule 13). An AC per axis is not coverage: state the order the axes compose in — normally an ordered pipeline — and add one AC per interaction the order makes observable. Checked **per pair**. Where several constants bound one quantity, state the satisfiability relation too. If pinning the interactions breaches the AC cap, split the story.
...
- **Pin every design mandate that names an API, or make the prose non-normative** (guide Rule 12). Design prose that constrains *how* the implementation must work by naming a symbol, API or call order — a "Library APIs used:" list, "X goes through `foo()` directly", "through A, **not** `B`", a numbered call sequence — is a normative contract. For each, either write an AC pinning the call observably, soften the prose to non-normative wording, or name it in the owning story's **Out of scope**. Sweep the *whole* Design section, not just `### Failure Handling`. **Fires on third-party and standard-library APIs too** — every other "must be invoked" guard here (two-anchor rule, seam ACs, unpaired-export blocker) triggers only on symbols this spec exports, so a dependency-API mandate has no other guard. **Mocked-collaborator corollary:** where the harness stubs the mandated dependency wholesale, an AC asserting only a downstream outcome is non-discriminating — the double gets shaped to whatever the implementation calls. Assert on the double's captured calls, or give the double the real contract (including the inputs on which it returns nothing). Unlike Rule 11, the planner does not backfill this: an unpinned mandate is reachable by no test and by exactly one detector — semantic review, which blocks on green code round after round.
...
- **New exported symbol from Track A** → pair with a **seam AC** (the **two-anchor rule**): the consumer's `[unit]`/`[integration]` test stubs the new symbol, triggers the consumer's production path **at the named outermost entry point** (route / command / event / tick — never an intermediate helper the feature introduces; see the seam-altitude rule in Phase 4), and asserts the symbol was invoked with the expected arguments. This proves *used*, not just *present*. If the wiring is guarded (once-per-transition/dedup), add the re-trigger AC that asserts a second trigger does not re-invoke. **When both the stubbed symbol and the entry point already exist** (neither is created by this spec), open the named entry point and confirm it actually reaches that symbol — including any enabling flag or guard on the path, and the specific method you are asserting on — *before* writing the AC. If a guard gates the path, either set it in the AC's own fixture or make it production config the story creates (a sibling AC declaring the option true) — and say which. If it does not reach at all, the AC describes wiring the story must first create: say so explicitly, or drop the AC. An AC asserting a call path that does not exist becomes an acceptance test that can never go green **and** a verbatim quote surface the adversarial reviewer can cite indefinitely.
...
6. Verify every `### Failure Handling` row traces to an AC in its owning story, or to that story's **Out of scope**. Do not add ACs for failure paths the design does not state.
7. Verify every seam AC whose stubbed symbol **and** entry point both already exist names an entry point that genuinely reaches it — path present, any gating flag turned on by the AC's fixture or by production config the story creates, and the named method actually called. (If either endpoint is new, the path is being created and there is nothing to check.)

**Blocker:** untagged AC; AC tagged `[grep]` / `[file]` / `[verbatim]`; AC expressed as a file-content / grep / "file contains" assertion; **any AC containing a shell command** (`grep`, `wc -l`, `find`, `awk`, `sed`, shell pipe `|`) **or a language-specific test API** (`readFileSync`, `expect()`, `assert "X" in ...`, `os.ReadFile`, etc.); unpaired new externally-visible symbol with no behavioral seam AC (as defined in Phase 4); **seam AC over two already-existing endpoints whose entry point does not reach the stubbed symbol**; removal keyword encoded as an AC instead of a build/static-gate note; aspirational meta-AC; **risk-sensitive story with a canonical risk property neither encoded as an AC nor declared out of scope** (adversarial-scope rule); **a `### Failure Handling` row with neither a covering AC nor an out-of-scope entry** (guide Rule 11); **a Design-prose mandate naming a specific API, symbol or call sequence with neither a covering AC nor an out-of-scope entry in its owning story** (guide Rule 12).

**Major:** **two or more thresholds/limits/modes on one function with nothing stating how they compose** (guide Rule 13) — blocker when two ACs read literally demand opposite outputs for one input.
...
| `` `grep -nE "PAT" path` returns 1 match `` | `[integration]` stub `<callee>`; trigger `<named entry point — route/command/event, above any wiring guard>`; assert `<callee>` called once with `<args>` (+ a re-trigger AC asserting no second call when the wiring is guarded). |
