...
- **State the composition when a function declares 2+ thresholds, limits or modes** (guide Rule 13). State the order the axes compose in — normally an ordered pipeline — and add one AC per interaction the order makes observable; checked **per pair**. Where several constants bound one quantity, state the satisfiability relation too.
