...
**Major:** under-specified input class; adversarial-scope gap on a risk-sensitive story; a `### Failure Handling` row with neither a covering AC nor an out-of-scope entry (the planner authors it instead, in its own words); a Design-prose API or call-sequence mandate with neither a covering AC nor an out-of-scope entry in its owning story.
