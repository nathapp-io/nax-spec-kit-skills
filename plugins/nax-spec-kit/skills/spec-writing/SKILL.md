...
- **New exported symbol from Track A** → pair with a **seam AC** (the **two-anchor rule**): the consumer's `[unit]`/`[integration]` test stubs the new symbol, triggers the consumer's production path **at the named outermost entry point** (route / command / event / tick — never an intermediate helper the feature introduces; see the seam-altitude rule in Phase 4), and asserts the symbol was invoked with the expected arguments. This proves *used*, not just *present*. If the wiring is guarded (once-per-transition/dedup), add the re-trigger AC that asserts a second trigger does not re-invoke. **When the seam AC stubs a symbol that already exists** (rather than one this story creates), open the named entry point and confirm it actually reaches that symbol — including any enabling flag or guard on the path, and the specific method you are asserting on — *before* writing the AC. If it does not reach, the AC describes wiring the story must first create: say so explicitly, or drop the AC. An AC asserting a call path that does not exist becomes an acceptance test that can never go green **and** a verbatim quote surface the adversarial reviewer can cite indefinitely.
