...
6. Verify every seam AC whose stubbed symbol **and** entry point both already exist names an entry point that genuinely reaches it — path present, any gating flag turned on by the AC's fixture or by production config the story creates, and the named method actually called. (If either endpoint is new, the path is being created and there is nothing to check.)
