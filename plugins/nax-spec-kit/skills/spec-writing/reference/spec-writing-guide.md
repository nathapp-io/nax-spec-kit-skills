...
10. **Pin every applicable risk property, or declare it out of scope.** When a story's subject matter is risk-sensitive — authentication/sessions, rate limiting/counters, replay protection (TOTP/OTP/MFA/nonce), multi-tenancy scoping, concurrency/atomicity (check-then-act, upsert, locks), expiry/TTL/retention, crypto/secrets — enumerate that domain's canonical risk properties (window expiry and atomic increment for rate limiters; reuse-within-window rejection for replay stores; tenant scoping on every read/write; expired-row exclusion; etc.) and for each one either write an executable AC or list it under **Out-of-scope** with a one-line reason. A risk property left silent is contested territory for the adversarial reviewer, which runs last on green code and can substantiate factually-true findings about the unpinned property while quoting an adjacent AC verbatim — every such finding survives AC-grounding and blocks, round after round (real case: an IAM-store story with 7 happy-path passthrough ACs looped ~18 adversarial rounds on atomic windows, TOTP replay derivation, and tenant nullability that no AC mentioned). The out-of-scope route is legitimate and cheap; the point is that the **spec** owns the scope boundary, not a downstream reviewer.
